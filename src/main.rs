...
impl MessageSender {
    ...
    async fn send_message_with_retry(
        ...
        for attempt in 0..=retries {
            ...
            let mut command = Command::new("meshcli");
...
async fn check_node_connection(args: Args) -> Result<()> {
    // Construct the command to run `meshcli`
    let mut cmd = Command::new("meshcli");

    // Conditionally add the "-t" argument if the host is provided
    if let Some(host) = &args.host {
        cmd.arg("-t");
        cmd.arg(host);  // Add host argument here
    }

    // Conditionally add the "-s" argument if the serial port is provided ie. /dev/ttyUSB0
    if let Some(port) = &args.port {
        cmd.arg("-s");
    ...
    match output {
        Ok(output) => {
            ...
            if let Some(first_line) = stdout.lines().next() {
                if first_line.contains("Interactive mode") {
                ...
                log::error!("Output from meshcli was empty.");
                std::process::exit(1);
            }
        ...
        Err(e) => {
            ...
            log::error!("Failed to execute meshcli: {}", e);
            std::process::exit(1);
        }
...
async fn main() -> Result<()> {
    ...
    SimpleLogger::new()
        ...
        .with_module_level("MeshCore_SAME_EAS_Alerter", LevelFilter::Info)
        .init()
        .unwrap();
    ...
    // 50 means that tests will not be logged
    let mut test_channel: u32 = 50;
    ...
    if let Some(alert_channel_arg) = args.alert_channel {
        if !(0..=39).contains(&alert_channel_arg) {
            return Err(anyhow::anyhow!("alertChannel must be between 0 and 39"));
        } else {
    ...
    if let Some(test_channel_arg) = args.test_channel {
        if !(0..=39).contains(&test_channel_arg) {
            return Err(anyhow::anyhow!("testChannel must be between 0 and 39"));
        }
    ...
    for msg in rx.iter_messages(audiosrc.map(|sa| sa as f32)) {
        match msg {
            Message::StartOfMessage(hdr) => {
                ...
                // Split and send the message in chunks of 140 characters, using retry logic
                ...
                for i in myvec.iter_mut() {
                    if curlen + *i - curpos > 140 {
                        sender
                            .send_message_with_retry(Args::parse(), send_channel, &message[startpos..(startpos + curlen)], 3, Duration::from_secs(5))
